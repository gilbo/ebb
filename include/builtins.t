...
--[[ Builtin macros                      ]]--
---------------------------------------------
function AssertCheck(ast, env, diag)
    local args = ast.params.children
    if #args ~= 1 then
        diag:reporterror(ast, "assert expects exactly 1 argument (instead got " .. #args .. ")")
        return
    end

    local test = args[1]
    local test_type = test:check(env, diag)
    if test_type ~= t.error and test_type ~= t.bool then
        diag:reporterror(ast, "Expected a boolean as the test for assert statement")
    end
end
...
function PrintCheck(ast, env, diag)
    local args = ast.params.children
    if #args ~= 1 then
        diag:reporterror(ast, "print expects exactly one argument (instead got " .. #args .. ")")
    end

    local output = args[1]
    local outtype = output:check(env, diag)
    if outtype ~= t.error and not outtype:isExpressionType() then
        diag:reporterror(ast, "Only numbers, bools, and vectors can be printed")
    end
end
...
function DotCheck(ast, env, diag)
    local args = ast.params.children
    if #args ~= 2 then
        diag:reporterror(ast, "dot expects exactly 2 arguments (instead got " .. #args .. ")")
        return
    end
    local lt1 = args[1]:check(env, diag)
    local lt2 = args[2]:check(env, diag)
    if not lt1:isVector() then
        diag:reporterror(args[1], "First argument to dot must be a vector")
    end
    if not lt2:isVector() then
        diag:reporterror(args[2], "Second argument to dot must be a vector")
    end
    if lt1.N ~= lt2.N then
        diag:reporterror(ast, "Cannot dot vectors of differing lengths (" ..
                lt1.N .. " and " .. lt2.N .. ")")
    end
    if lt1:baseType() == t.bool then
        diag:reporterror(args[1], "Boolean vector passed as first argument to dot")
    end
    if lt2:baseType() == t.bool then 
        diag:reporterror(args[2], "Boolean vector passed as second argument to dot")
    end
