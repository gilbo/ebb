...
--
local VERSION = 20131004.7  -- version history at end of file
...
function encode_pretty_value(self, value, parents, indent, etc)
   ...
   elseif type(value) ~= 'table' then
      ...
      if not object_keys then
         ...
         local PARTS = { }
         local indent_plus = indent.."  "
         for _, key in ipairs(object_keys) do
            local encoded_key = encode_pretty_value(self, tostring(key),
                                                    parents, "", etc)
            local encoded_val = encode_pretty_value(self, T[key], parents,
                                                    indent_plus, etc)
            table.insert(PARTS, string.format(indent_plus.."%s: %s",
