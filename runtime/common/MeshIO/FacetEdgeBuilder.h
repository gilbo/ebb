...
#define IS_HOLE(fe) (VALUE(fe) != VALUE(NEXT(fe)->flip()))
	...
	void dump(size_t N, uint32_t * ids) {
		map_t & cells = cellMap();
		
		if(!ids) {
		for(map_t::iterator it = cells.begin(),end = cells.end(); it != end; it++) {
			printf("%d %d %d\n",(int)it->first,(int)it->second->ID(),(int)it->second->isOther());
		}
		} else {
		for(size_t i = 0; i < N; i++) {
			HalfFacet * hf = cells[ids[i]];
			printf("%d %d %d\n",(int)ids[i],hf->ID(),(int)hf->isOther());
		}
		}
	}
private:
	...
	bool setupFE(id_t id, const FileFacetEdge * ffe, FacetEdge * fe) {
		...
		if(addToRing(0,&fe->hf[0])) { //rings use hf[0] since this was the default direction listed in the files,
			...
			addToMap(vertMap(),hf->vert(),hf,false);
			addToMap(cellMap(),hf->cell(),hf,false);
	...
	bool addToRing(int type, HalfFacet * hf) {
		...
		addToMap(elemMap,elemID,hf,true);
...
#undef IS_HOLE
	...
	void filterFacet(Filter & filter, FacetEdge * fe) {
		...
		addToMap(edgeMap(),fe->edge_face[0],&fe->hf[0],true);
		addToMap(faceMap(),fe->edge_face[1],&fe->hf[0],true);
	}
public:
	typedef std::hash_map<id_t,HalfFacet *> map_t;
private:
	void addToMap(map_t & m, id_t key, HalfFacet * hf, bool isEdgeOrFace) {
		map_t::iterator iter = m.find(key);
		if(iter == m.end()) {
		m[key] = hf;
		} else {
		//ensure the canonical HF is the first seen for edges or faces, but the 
		//last seen for cells/vertices
		//this matches the old behavior but will make it consistent even when the mesh is partitioned and renumbered
		HalfFacet * cur = iter->second;
		if(cur->ID() > hf->ID()) {
			if(isEdgeOrFace) {
			m[key] = hf;
			}
