...
grid.cells:NewField('index', L.vector(L.int, 2))
grid.cells.index:LoadConstant(L.NewVector(L.int, {0,0}))
...
--    var x = c.index[1] - dt0 * c.velocity[1]
    var x = 0
...
--    var y = c.private.index[2] - dt0 * c.velocity[2]
    var y = 0
...
local project_update_prev = liszt_kernel(c : grid.cells)
...
--    project_preprocess()
    ...
    project_update_prev(grid.cells)
