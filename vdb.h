// This copy of vdb.h has been doctored up to interoperate with Terra
// instead of giving errors.  In the process it lost access
// to callsite labeling/filtering.
...
//}
VDB_CALL int vdb_point_v(void * p) {
  return vdb_point_fn(1,p,3*sizeof(float));
}
VDB_CALL int vdb_point(float x, float y, float z) {
  float p[] = {x,y,z};
...
//}
VDB_CALL int vdb_line_v(void * p) {
  return vdb_line_fn(1,p,6*sizeof(float));
}
...
//}
VDB_CALL int vdb_normal_v(void * p) {
  return vdb_normal_fn(1,p,6*sizeof(float));
}
...
//}
VDB_CALL int vdb_triangle_v(void * p) {
  return vdb_triangle_fn(1,p,9*sizeof(float));
}
