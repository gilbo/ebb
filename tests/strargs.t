...
local liszt check0(c : cells, field)
  var fval = c[field]
  L.assert(fval == 0)
end
local liszt check1(c : cells, field)
  var fval = c[field]
  L.assert(fval == 1)
end

local liszt copyfield(c : cells, fdst, fsrc)
  c[fdst] = c[fsrc]
end

local liszt noargs(c : cells) end
...
-----------------------------------
test.fail_function(function()
  local liszt test(c : cells, val : L.double)
    c.temp = val
  end
...
---------------------------------
test.fail_function(function()
  local liszt test(c : cells, field)
    c[0] = 2
  end
  cells:foreach(test, 'temp')
...
-----------------------------------
local liszt checktemp0(c : cells)
  var fval = c['temp']
  L.assert(fval == 0)
end
...
local pres_str = 'pres'
local liszt checkpres0(c : cells)
  var fval = c[pres_str]
  L.assert(fval == 0)
end
...
local liszt passthrough(c : cells, field)
  check0(c, field)
end
...
local liszt macropassthrough(c : cells, field)
  m0check(c, field)
end
